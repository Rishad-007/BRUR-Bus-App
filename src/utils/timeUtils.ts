...
export const isBusAvailableToday = (
  day: string,
  specialConditions: any
): { available: boolean; message?: string } => {
  if (specialConditions[day]) {
    return {
      available: specialConditions[day].available,
      message: specialConditions[day].message || "",
    };
  }
  return { available: true };
};

export const getLimitedSchedule = (
  day: string,
  specialConditions: any
): any[] => {
  if (specialConditions[day] && specialConditions[day].limitedSchedule) {
    return specialConditions[day].limitedSchedule;
  }
  return [];
};
