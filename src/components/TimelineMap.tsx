...
export default function TimelineMap({ stops, selectedStopId, onStopSelect, routeName }: TimelineMapProps) {
        ...
        <div className="space-y-4 pr-2">
          {stops.map((stop, index) => {
            ...
            return (
              ...
              >
                {/* Timeline dot */}
                <div className={`relative z-10 w-12 h-12 rounded-full flex items-center justify-center border-4 transition-all duration-200 ${
                  isSelected 
                  ...
                  <MapPin className={`w-5 h-5 ${
                    isSelected ? 'text-white' : 'text-gray-500 group-hover:text-university-600'
                  }`} />
                </div>
                
                {/* Stop info */}
                <div className={`ml-4 flex-1 p-3 rounded-lg transition-all duration-200 ${
                  isSelected 
                  ...
                  <h4 className={`font-medium ${
                    isSelected ? 'text-university-800' : 'text-gray-800 group-hover:text-university-700'
                  }`}>
                    {stop.name}
                  </h4>
                  <p className={`text-sm ${
                    isSelected ? 'text-university-600' : 'text-gray-500 group-hover:text-university-500'
                  }`}>
